...
import {AOSServiceManager, IServiceManager} from "../src/AOSServiceManager.sol";
import {AOSChallengeManager} from "../src/AOSChallengeManager.sol";
import {IAOSChallengeManager} from "../src/IAOSChallengeManager.sol";
...
contract AOSDeployer is Script, Utils {
    ...
    ProxyAdmin public aOSProxyAdmin;
    PauserRegistry public aOSPauserReg;
    ...
    AOSServiceManager public aOSServiceManager;
    IServiceManager public aOSServiceManagerImplementation;

    AOSChallengeManager public aOSChallengeManager;
    IAOSChallengeManager
        public aOSChallengeManagerImplementation;
    ...
    function _deployCredibleSquaringContracts(
        ...
        aOSProxyAdmin = new ProxyAdmin();

        // deploy pauser registry
        {
            address[] memory pausers = new address[](2);
            pausers[0] = credibleSquaringPauser;
            pausers[1] = aOSCommunityMultisig;
            aOSPauserReg = new PauserRegistry(
                pausers,
                aOSCommunityMultisig
            );
        }
         ...
         */
        aOSServiceManager = AOSServiceManager(
            address(
                new TransparentUpgradeableProxy(
                    address(emptyContract),
                    address(aOSProxyAdmin),
                    ""
                )
            )
        );
        aOSChallengeManager = AOSChallengeManager(
            address(
                new TransparentUpgradeableProxy(
                    address(emptyContract),
                    address(aOSProxyAdmin),
        ...
        aOSServiceManagerImplementation = new AOSServiceManager(
            avsDirectory,
            registryCoordinator,
            stakeRegistry
        );
        ...
        aOSProxyAdmin.upgrade(
            TransparentUpgradeableProxy(
                payable(address(aOSServiceManager))
            ),
            address(aOSServiceManagerImplementation)
        );

        aOSChallengeManagerImplementation = new AOSChallengeManager(
            registryCoordinator,
        ...
        aOSProxyAdmin.upgradeAndCall(
            TransparentUpgradeableProxy(
                payable(address(aOSChallengeManager))
            ),
            address(aOSChallengeManagerImplementation),
            abi.encodeWithSelector(
                aOSChallengeManager.initialize.selector,
                aOSPauserReg,
                aOSCommunityMultisig,
                AGGREGATOR_ADDR,
                TASK_GENERATOR_ADDR
        ...
        vm.serializeAddress(
            deployed_addresses,
            "credibleSquaringChallengeManager",
            address(aOSChallengeManager)
        );
        vm.serializeAddress(
            deployed_addresses,
            "credibleSquaringChallengeManagerImplementation",
            address(aOSChallengeManagerImplementation)
        );
        vm.serializeAddress(
            deployed_addresses,
